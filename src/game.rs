...
/// e.g. consume all the events out of the `.mouse_button_events` vector.
...
impl Game {
    /// Create an empty [`Game`] with an empty [`EngineState`] and an empty vector of [`Actor`]s
    fn new() -> Self {
    ...
    fn add_actor<T: Into<String>>(&mut self, label: T, preset: ActorPreset) -> &mut Actor {
        let label = label.into();
        self.engine_state
            .actors
            .insert(label.clone(), preset.build(label.clone()));
        // Unwrap: Can't crash because we just inserted the actor
        self.engine_state.actors.get_mut(&label).unwrap()
    }

    #[must_use]
    /// Add a [`TextActor`] before the game has begun. Use the `&mut TextActor` that is returned to
    /// set the translation, rotation, etc.
    fn add_text_actor<T, S>(&mut self, label: T, text: S) -> &mut TextActor
    where
        T: Into<String>,
        S: Into<String>,
    {
        ...
        // Unwrap: Can't crash because we just inserted the actor
        self.engine_state.text_actors.get_mut(&label).unwrap()
    }
    ...
    ///
    /// fn game_logic(engine_state: &mut EngineState) {
    ///     println!("This game has been running for: {:.2}", engine_state.time_since_startup_f64);
    /// }
    ///
    /// fn main() {
    ...
    /// ```
    ///
    /// You can use a closure for your game logic, if and only if your closure doesn't capture
    /// any variables by reference.
    ///
    /// ```no_run
    /// # use rusty_engine::prelude::*;
    /// let mut game = Game::new();
    /// game.run(|engine_state| {
    ///     println!("This game has been running for: {:.2}", engine_state.time_since_startup_f64);
    /// });
    /// ```
    ///
    ...
    fn run(&mut self, initial_game_state: $game_state_type) {
        ...
        self.app_builder
            ...
            .add_startup_system(rusty_engine::prelude::setup.system());
...
fn game_logic_sync(
    ...
    rusty_engine::prelude::add_actors(&mut commands, &asset_server, materials, &mut engine_state);

    // Add Bevy components for any new text_actors remaining in engine_state.text_actors
    rusty_engine::prelude::add_text_actors(&mut commands, &asset_server, &mut engine_state);
